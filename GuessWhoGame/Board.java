...
public class Board{
    ...
    public Board(){
        ...
        for (int r=0; r<board.length; r++){
            for(int c=0; c<board[r].length; c++){
                ...
                this.board[r][c] = boardItems.get(boardNum);
    ...
    //sets the board(not used for anything but testing)
   ...
   //checks what question is being asked and updates the board to eliminate all characters that don't fit the description
   public static void updateBoard(int questionAsked, Boolean yesAnswer){
       for(Tile[] r: board){
           for (Tile c: r){
                ArrayList<String> tileCharacteristics = c.getInputArr();
                if(!yesAnswer){
                    //a blank string means the players character does not have that attribute so if the answer to the question is yes 
                    //all characters with that attribute need to be eliminated
